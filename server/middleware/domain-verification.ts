...
export const requireDomainVerification = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    if (!domain) {
      return res.status(403).json({
        success: false,
        error:
          "Domain verification required. Please verify your domain ownership first.",
      });
    }
...
export const checkDomainForScript = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    if (!verification) {
      return res.status(403).json({
        ...
        message:
          "Please verify your domain ownership to use PriceHunt tracking scripts.",
      });
    }
