  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        error:
          "Missing required fields: orderId, businessId, productUrl, productPrice",
      });
    }
  ...
  try {
    ...
    if (!status || !Object.values(SaleStatus).includes(status)) {
      return res.status(400).json({
        success: false,
        error:
          "Invalid status. Must be one of: PENDING, CONFIRMED, CANCELLED, REFUNDED",
      });
    }
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to update sale status" });
  }
});
...
router.put("/commission/paid/:saleId", requireAuth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to mark commission paid" });
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (isNaN(businessIdNum)) {
        return res
          .status(400)
          .json({ success: false, error: "Invalid business ID" });
      }

      const start = startDate ? new Date(startDate as string) : undefined;
      const end = endDate ? new Date(endDate as string) : undefined;

      const stats = await SalesTrackingService.getBusinessSalesStats(
        businessIdNum,
        start,
        end,
      );
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to get sales statistics" });
    }
  },
);
...
router.get("/stats/commissions", requireAuth, async (req, res) => {
  try {
    ...
    const stats = await SalesTrackingService.getUserCommissionStats(
      userId,
      start,
      end,
    );
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to get commission statistics" });
  }
});
...
router.get("/commissions", requireAuth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to get commissions" });
  }
});
...
router.post("/commission-rates", requireBusinessAuth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to set commission rate" });
  }
});
...
router.get("/commission-rates", requireBusinessAuth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to get commission rates" });
  }
});
...
router.post("/admin/retry-webhooks", requireAdmin, async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Webhook retry process completed" });
  } catch (error) {
