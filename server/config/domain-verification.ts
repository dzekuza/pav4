...
export const DOMAIN_VERIFICATION_CONFIG = {
  ...
  WARNING_MESSAGE: "Domain verification is recommended for enhanced features and accurate tracking.",
  
  // Success message when domain is verified
  SUCCESS_MESSAGE: "Domain verified successfully! You now have access to all features.",
};
...
export function requiresDomainVerification(feature: string): boolean {
  ...
  return DOMAIN_VERIFICATION_CONFIG.FEATURES_REQUIRING_VERIFICATION.includes(feature);
}

// Helper function to check if a feature is available without verification
export function isFeatureAvailableWithoutVerification(feature: string): boolean {
  return DOMAIN_VERIFICATION_CONFIG.FEATURES_AVAILABLE_WITHOUT_VERIFICATION.includes(feature);
}
...
export function getAvailableFeatures(isDomainVerified: boolean): string[] {
  const availableFeatures = [...DOMAIN_VERIFICATION_CONFIG.FEATURES_AVAILABLE_WITHOUT_VERIFICATION];
  
  if (isDomainVerified) {
    availableFeatures.push(...DOMAIN_VERIFICATION_CONFIG.FEATURES_REQUIRING_VERIFICATION);
  }
...
export function isDashboardAccessAllowed(isDomainVerified: boolean = false): boolean {
  if (!DOMAIN_VERIFICATION_CONFIG.REQUIRED_FOR_DASHBOARD_ACCESS) {
    return true;
  }
