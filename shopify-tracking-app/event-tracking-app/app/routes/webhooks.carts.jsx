...
export const action = async ({ request }) => {
  ...
  try {
    ...
    if (result.success) {
      console.log(`Cart webhook processed successfully: ${topic} for shop: ${shop}`);
      return new Response(null, { status: 200 });
    } else {
      console.error(`Cart webhook forwarding failed: ${topic} for shop: ${shop}`, result.error);
      // Still return 200 to Shopify to prevent retries, but log the error
      return new Response(null, { status: 200 });
    }
  } catch (error) {
    console.error(`Error processing cart webhook: ${topic} for shop: ${shop}`, error);
