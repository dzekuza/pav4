...
async function checkBusinessAffiliateIds() {
  try {
    ...
    const businessesWithAffiliateIds = businesses.filter(
      (b) => b.affiliateId && b.affiliateId.trim() !== "",
    );
    const businessesWithoutAffiliateIds = businesses.filter(
      (b) => !b.affiliateId || b.affiliateId.trim() === "",
    );

    console.log(`\n📊 Affiliate ID Status:`);
    console.log(
      `✅ Businesses with affiliate IDs: ${businessesWithAffiliateIds.length}`,
    );
    console.log(
      `❌ Businesses without affiliate IDs: ${businessesWithoutAffiliateIds.length}`,
    );
    ...
    if (businessesWithAffiliateIds.length > 0) {
      ...
      businessesWithAffiliateIds.forEach((business) => {
        console.log(
          `  - ${business.name} (ID: ${business.id}, Domain: ${business.domain})`,
        );
    ...
    if (affiliateIds.length !== uniqueAffiliateIds.size) {
      ...
      const duplicates = affiliateIds.filter(
        (id, index) => affiliateIds.indexOf(id) !== index,
      );
