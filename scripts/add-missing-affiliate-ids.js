...
async function addMissingAffiliateIds() {
  try {
    ...
    console.log(`Found ${businessesWithoutAffiliateIds.length} businesses without affiliate IDs`);
    ...
    for (const business of businessesWithoutAffiliateIds) {
      ...
      if (attempts >= maxAttempts) {
        console.error(`Failed to generate unique affiliate ID for business ${business.id} (${business.name})`);
        continue;
      }
      ...
      console.log(`Updated business "${business.name}" (ID: ${business.id}) with affiliate ID: ${affiliateId}`);
    }

    console.log('Successfully added affiliate IDs to all businesses!');

    // Verify all businesses now have affiliate IDs
    const remainingBusinessesWithoutAffiliateIds = await prisma.business.findMany({
      where: {
        OR: [
          { affiliateId: null },
          { affiliateId: '' }
    ...
    if (remainingBusinessesWithoutAffiliateIds.length === 0) {
      console.log('✅ Verification successful: All businesses now have affiliate IDs');
    } else {
      console.log(`❌ Warning: ${remainingBusinessesWithoutAffiliateIds.length} businesses still don't have affiliate IDs`);
    }
