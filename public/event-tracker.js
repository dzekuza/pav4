...
(function () {
  ...
  function initCustomEventTracking() {
    ...
    document.addEventListener("click", function (e) {
      ...
      if (
        target.matches(
          '[data-track="buy-now"], .buy-now, .purchase-now, [class*="buy"], [class*="purchase"], .btn-buy, .btn-purchase',
        )
      ) {
        const productId =
      ...
      if (
        target.matches(
          '[data-track="view-product"], .product-link, [class*="product"], a[href*="product"]',
        )
      ) {
        const productId =
          target.getAttribute("data-product-id") ||
      ...
      if (
        target.matches(
          '[data-track="add-to-cart"], .add-to-cart, [class*="cart"], [class*="add"]',
        )
      ) {
        const productId =
          target.getAttribute("data-product-id") ||
    ...
    document.addEventListener("submit", function (e) {
      ...
      if (
        form.matches(
          '[data-track="lead-form"], .contact-form, .newsletter-form',
        )
      ) {
    ...
    document.addEventListener("click", function (e) {
      ...
      if (
        ...
        window.trackEvent("social_media_click", {
          platform: target.href.includes("facebook")
            ? "facebook"
            : target.href.includes("twitter")
              ? "twitter"
              : target.href.includes("instagram")
                ? "instagram"
                : target.href.includes("linkedin")
                  ? "linkedin"
                  : "other",
          url: target.href,
        });
