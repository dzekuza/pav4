  ...
  function initWooCommerceTracking() {
    ...
    document.addEventListener("click", function (e) {
      ...
      if (
        ...
        const productName =
          document.querySelector(".product_title")?.textContent ||
          target.closest(".product")?.querySelector(".product_title")
            ?.textContent;
        const priceElement = document.querySelector(
          ".price .amount, .woocommerce-Price-amount",
        );
      ...
      if (
        target.matches(
          '.buy-now, .purchase-now, [class*="buy"], [class*="purchase"]',
        )
      ) {
        const productId =
          target.getAttribute("data-product_id") ||
    ...
    if (
      ...
      const orderId =
        document.querySelector(".woocommerce-order-number")?.textContent ||
        document
          .querySelector("[data-order-id]")
          ?.getAttribute("data-order-id");
      const totalAmount =
        document.querySelector(".woocommerce-order-total .amount")
          ?.textContent ||
    ...
    document.addEventListener("click", function (e) {
      ...
      if (
        target.matches(
          'a[href*="/product/"], .woocommerce-loop-product__link, .product a',
        )
      ) {
        const productId =
          target.getAttribute("data-product_id") ||
          target.closest("[data-product_id]")?.getAttribute("data-product_id");
        const productName =
          target.querySelector(".woocommerce-loop-product__title")
            ?.textContent || target.getAttribute("data-product-name");
      ...
      jQuery(document).on(
        ...
        function (event, fragments, cart_hash, button) {
          ...
          const productName = button
            .closest(".product")
            ?.querySelector(".woocommerce-loop-product__title")?.textContent;
