...
(function () {
    ...
    fetch(CONFIG.apiEndpoint, {
      ...
      .then((response) => {
        if (response.ok) {
          if (CONFIG.debug) {
            console.log(
              "[PriceHunt] Event sent successfully:",
              eventData.event_type,
            );
          }
        } else {
          console.error(
            "[PriceHunt] Failed to send event:",
            response.status,
            response.statusText,
          );
        }
  ...
  function initializeTracking() {
    if (CONFIG.debug) {
      console.log(
        "[PriceHunt] Initializing self-hosted tracking for business:",
        CONFIG.businessId,
      );
    }
    ...
    if (CONFIG.debug) {
      ...
      console.log(
        "[PriceHunt] Available functions: PriceHuntTracker.track(), PriceHuntDebug.getTrackerStatus()",
      );
    }
  }
