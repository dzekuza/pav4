...
(function () {
  ...
  mainScript.setAttribute(
    "data-business-id",
    document.currentScript.getAttribute("data-business-id"),
  );
  mainScript.setAttribute(
    "data-affiliate-id",
    document.currentScript.getAttribute("data-affiliate-id"),
  );
  ...
  function initMagentoTracking() {
    ...
    document.addEventListener("click", function (e) {
      ...
      if (
        target.matches(
          '.action.primary.tocart, .btn-cart, [data-action="add-to-cart"]',
        )
      ) {
        const productId =
        ...
        const productName =
          document.querySelector(".page-title")?.textContent ||
          target.closest(".product-info")?.querySelector(".page-title")
            ?.textContent;
        const priceElement = document.querySelector(
          ".price-box .price, .special-price .price",
        );
      ...
      if (
        target.matches(
          '.action.primary.checkout, .btn-buy-now, [data-action="buy-now"]',
        )
      ) {
        const productId =
          target.getAttribute("data-product-id") ||
    ...
    if (
      document.body.classList.contains("checkout-onepage-success") ||
      document.body.classList.contains("checkout-success")
    ) {
      const orderId =
        document.querySelector(".checkout-success .order-number")
          ?.textContent ||
        document
          .querySelector("[data-order-id]")
          ?.getAttribute("data-order-id");
      const totalAmount =
        document.querySelector(".checkout-success .grand-total .price")
          ?.textContent ||
    ...
    document.addEventListener("click", function (e) {
      ...
      // Track product links
      if (
        target.matches('a[href*="/product/"], .product-item-link, .product a')
      ) {
        const productId =
          target.getAttribute("data-product-id") ||
          target.closest("[data-product-id]")?.getAttribute("data-product-id");
        const productName =
          target.querySelector(".product-item-name")?.textContent ||
          target.getAttribute("data-product-name");
